    ...
    <View style={{ gap: compact ? 12 : 16 }}>
      {groups.map((g) => (
        <View key={g.key} style={{ gap: 6 }}>
          ...
          {g.skills.map((s) => (
            <View key={s.key} style={[styles.skillRow, s.isSpec && { paddingLeft: 14 }]}>
              <Text
                numberOfLines={1}
                style={{
                  flex: 1,
              ...
              <View style={styles.skillVals}>
                <Text
                  style={{ fontFamily: 'Cinzel_600SemiBold', fontSize: 12.5, color: g.color, minWidth: 12, textAlign: 'right' }}>
                  {s.value}
                </Text>
                ...
                <View style={[styles.totalBadge, { backgroundColor: g.color }]}>
